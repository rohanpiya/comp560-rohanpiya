...
#  The model is trained to fill those K positions with whatever it finds useful.
#  Cross-entropy is applied over all positions including the underscores.
# ─────────────────────────────────────────────
...
def _scratch(a: int, b: int, k: int) -> str:
    """
    Build a scratch-format training example for a+b using k scratch tokens.
    The model must output k underscores then the answer.
